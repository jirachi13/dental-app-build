...
# Sprints 28–102 and session notes — moved from HANDOFF.md 2026-09-04

Moved wholesale in the hygiene pass CLAUDE.md requires every ~5 sprints; HANDOFF.md had
reached 1817 lines, 60% of it finished work, and it is injected into every session.
Content is verbatim apart from one correction: the Sprint A–J headings said
**"ON BRANCH `dashboard-top-row`, NOT MERGED"**, which was stale — that branch merged to
`main` as `36eefb8b` (verified with `git branch --contains`), so those markers now say so.

Newest first, in the order they stood in HANDOFF.
...
- **VERIFIED 5/5 against the DEPLOYED build**: dentist and aide now list all 3 schools (the reported bug), schooladmin still exactly 1, `school_id` absent from `/auth/me` for every account. DB re-checked raw: 0 docs carry `school_id`, 11 carry `school_ids`.
- ⚠⚠ **MY FIRST VERIFICATION RUN PASSED AGAINST THE OLD BUILD, and it looked plausible.** The deploy-wait script polled until `x-vercel-id` showed `sin1` — which was ALREADY true from Sprint 99, so it returned in 0 s and the checks ran against the pre-deploy function. It still printed sensible `school_ids` because **Mongoose hydration does not strip fields that are absent from the schema**, so the migrated array leaked through the old model, while the old schema's `default: null` supplied a phantom `school_id: null`. The tell was that one contradictory column. **Wait on a string the NEW build introduced (`whichbuild.mjs` greps the live bundle for "Specific schools"), never on a property that predates it.** Same family as the "suspect the verifier first" rule.
...
## Sprint 102 (Promote/Assign is re-runnable) — DONE 2026-09-04, tsc + build clean, 7/7 verified
**The screen that made a placement mistake could not fix it.** `PromoteAssign.tsx` forced `action: 'skip'` for any pupil already holding the target year and disabled the row, because a second POST 409s on `uniqueBy: ["student_id","school_year"]`. So a wrong section, a wrong grade, or a retain that should have been a promote could only be undone by editing every pupil by hand (Sprint 70) — the exact work this screen exists to remove.

- **A fourth action, `update`:** those rows now `PUT /student-iptrs/:id` instead of POSTing a duplicate. `create` and `correct` are counted and reported separately.
- **⚠ IT DEFAULTS TO SKIP, NOT UPDATE, AND THAT IS THE DESIGN.** Someone may have hand-corrected a pupil's year record; defaulting to update would silently stamp over it on the next run — trading a visible failure for an invisible one. **Same lesson as Sprint 101's `$and` bug: wrong data inside the boundary is worse than a clean error.** Correcting is opt-in per pupil, and a choice already made in the session survives the re-render.
- **The opt-in is never blind:** the row prints what the target year says NOW (`2027-2028 already: Grade 4 · Sampaguita`) and, once `update` is chosen, the proposed value in amber. The footer says plainly how many records will be **OVERWRITTEN**, and the button changes to "Correct N records" when nothing is being created.
- **`update` keeps the grade from the EXISTING record**, not from `target` — re-deriving it would quietly bump anyone corrected twice.
- **`node verify_sprint102.mjs` — 7/7.** Proves the whole point in one run: POST creates (201) → a duplicate POST is still refused (409, the original defect) → PUT corrects in place (200, section Sampaguita→Rosal) → still exactly one record for that year → correction stuck. Uses a throwaway school year `2098-2099` and archives it afterwards (never hard delete), then asserts nothing is left in live data.
- Only `PromoteAssign.tsx` changed. No schema, no migration, no server change — the PUT route and `uniqueBy` already existed and were not touched.
- **Still NOT done, deliberately (from the sprint scope):** the apply loop is still **2 requests per pupil, sequentially** (base44 does a whole cohort in a constant 5). At ~70-150 ms/request a 40-pupil section is ~10 s of serial round trips. A bulk endpoint belongs with backlog #39's consolidation work, not here.

## Sprint 101 (the school gate is enforced on the SERVER) — DONE 2026-09-04, 12/12 verified against production
`school_ids` rode in the JWT and **nothing read it**. The only school filtering came from `?school` — the client's own choice — so a school_admin pinned to Annex A received all three schools' students. The switcher was a display filter, not a boundary.

- **`server/utils/schoolScope.ts`** resolves a user's schools to each model's own key, walking the chain where there is no direct `school_id`: `StudentIptr` via `student_id`, the six IPTR children via `iptr_id`, `ToothRecord` via `chart_id` (3 levels), `RiskStratification` via `preventive_id`. Memoised per request so a request touching several levels resolves each once.
- **Applied to list, read-one, create, update, archive AND restore** in `crudFactory`, plus the three hand-written `/stats` routes. `/predictions` was checked and reads no student data.
- **Fails CLOSED**: a model with no rule in the table returns nothing rather than everything, so a model added later cannot silently become world-readable.
- **Out-of-scope reads return 404, not 403** — matching how archived records are hidden. A 403 would confirm the record exists.
- **Unscoped users (empty `school_ids`) run ZERO extra queries** — `scopeFilter` returns null immediately.
- **`npm run` — `node verify_sprint101.mjs`** is the original probe turned regression test: **12/12 PASS**. schooladmin 6/26 students in 1 school, iptrs 8/32, tooth records 39/177, risk 6/28; another school's student by id → 404, own → 200; dentist and bho still 26/26.

**⚠ THE BUG THE VERIFIER CAUGHT, and it was worse than the symptom it reported.** The failing check was cosmetic-looking (`?school` on another school returned the caller's own count). The cause was that the scope clause was merged by SPREADING: `{ ...filter, ...scope }`. **The scope keys on the SAME fields as `filterable`** — `student_id`, `iptr_id`, `chart_id` — so the spread silently DROPPED the caller's filter. `GET /medical-histories?iptr_id=X` would have returned every in-scope medical history instead of that pupil's: **one child's medical record rendered under another child's name.** Never a leak past the school boundary, but wrong data inside it, which is worse than a visible error. Fixed by `$and`-ing the two clauses; both cases are now regression-tested. **Never merge a permission clause into a query by spreading — the keys collide.**

**⚠ SCALING, recorded not hidden:** each chain level materialises an id list and filters with `$in`. Trivial at 26 students; at the Chapter 1 scale of ~8,000 a scoped user's list is thousands of ObjectIds per request. The durable fix is denormalising `school_id` onto the child models — a schema change across eight models plus a migration and write-path upkeep — deliberately NOT done here. Only scoped users pay it, and today that is school_admin alone.

**STILL OPEN, deliberately out of scope:** CLAUDE.md says *"School Administrator — view school reports + dashboards only, no clinical records"*, but a school_admin still reaches clinical records — now only for their own school. **That is a ROLE-permission question, not a school-scoping one**, and it needs its own decision: their dashboard reads clinical data to compute the very figures they are entitled to, so a blunt block breaks the screens they are supposed to have.

## 5th session recap — Sprints 89–98 (2026-09-03, superseded as the resume point by the 6th session below)
...
## Sprint A (dentist clinic summary strip, design 3a) — DONE 2026-08-11, LIVE-VERIFIED. ON BRANCH `dashboard-top-row`, since MERGED to main as `36eefb8b`
...
## Sprint B (DESIGN.md — blue carries operational state) — DONE 2026-08-11. ON BRANCH `dashboard-top-row`, since MERGED to main as `36eefb8b`
...
## Sprint C (bar labels out of the clipping track) — DONE 2026-08-11. ON BRANCH `dashboard-top-row`, since MERGED to main as `36eefb8b`
...
## Sprints D–G (the other role dashboards + the last donut) — DONE 2026-08-11, ALL FOUR ROLES LIVE-VERIFIED. ON BRANCH `dashboard-top-row`, since MERGED to main as `36eefb8b`
...
## Sprints G–H (school admin charts) — DONE 2026-08-11, LIVE-VERIFIED. ON BRANCH `dashboard-top-row`, since MERGED to main as `36eefb8b`
...
## Sprints I–J + dental-chart polish — DONE 2026-08-11. ON BRANCH `dashboard-top-row`, since MERGED to main as `36eefb8b`
