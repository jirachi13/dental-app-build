...
- Sprint 24 → DONE 2026-07-05: DOH Consolidated Report — added Excel (.xlsx) export (exceljs, 3-tier merged header, frozen Indicator col + headers, landscape print titles) AND reworked the PDF from the illegible 77-col-on-one-page into a single high-res zoomable whole-report page (SCALE auto-capped under the ~16384px canvas limit, JPEG). Excel = print/analyze, PDF = zoomable snapshot. Summary + footer capture fixed; footer sticky so it doesn't scroll on screen. Deployed + user-verified
