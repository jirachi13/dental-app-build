...
export function useAppointments() {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const [appointments, students, apiDentists, schools] = await Promise.all([
      ...
      const dentistNameById = new Map(apiDentists.map((d) => [d._id, `Dr. ${d.first_name} ${d.last_name}`]));
      ...
      for (const appt of appointments) {
        ...
        const key = [date, time, school, student.grade_level, student.section, appt.appointment_type, appt.dentist_id].join('|');
      ...
      setSessions(Array.from(groups.values()));
