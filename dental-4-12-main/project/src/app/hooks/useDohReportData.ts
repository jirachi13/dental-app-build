...
// rows with no matching schema field) stays on the original illustrative
// sparse table in Reports.tsx — see HANDOFF.md Sprint 13 notes.
...
export function useDohReportData() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(err instanceof Error ? err.message : 'Failed to load DOH report data');
      } finally {
  ...
  return { getRealCount, loading, error };
}
